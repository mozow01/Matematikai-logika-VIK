...
Inductive ternTree : Set :=
  | leaf0 : ternTree
  | node2 : ternTree -> ternTree -> ternTree
  | node3 : ternTree -> ternTree -> ternTree -> ternTree.

Definition ex_tree3 := node3 leaf0 (node2 leaf0 leaf0) (node3 leaf0 (node2 leaf0 leaf0) leaf0) .

Print binTree_ind.

Print nat_rect.

Definition ex_tree := node leaf (node leaf leaf).
...
Fixpoint leafLength (t : binTree) : nat :=
  match t with 
    | leaf => 1
